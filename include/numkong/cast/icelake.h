...
/** Convert 32x e2m3 → 32x bf16 via 32-entry LUT lookup (AVX-512BW). E2M3 format: S EE MMM (bias=1,
...
/** Widen 32x bf16 → 64 bytes of 32x f32 by shifting each half-word into the high f32 bits. Exact
...
/** Convert 32x e2m1 (16 packed bytes) → 32x f32 via a 16-entry BF16 LUT widened to f32. Input
 *  nibble order matches the serial codec: byte i holds element 2i in the high nibble and element
