...
/** Native WASM SIMD 128-bit load. */
NUMKONG_HELPER_INLINE void nk_load_b128_v128_(void const *src, nk_b128_vec_t *dst) { dst->v128 = wasm_v128_load(src); }

/** Native WASM SIMD 256-bit load using two v128 loads. */
NUMKONG_HELPER_INLINE void nk_load_b256_v128_(void const *src, nk_b256_vec_t *dst) {
    dst->v128s[0] = wasm_v128_load(src);
    dst->v128s[1] = wasm_v128_load((char const *)src + 16);
}

/** Native WASM SIMD 128-bit store. */
NUMKONG_HELPER_INLINE void nk_store_b128_v128_(nk_b128_vec_t const *src, void *dst) { wasm_v128_store(dst, src->v128); }

/** Native WASM SIMD 256-bit store using two v128 stores. */
NUMKONG_HELPER_INLINE void nk_store_b256_v128_(nk_b256_vec_t const *src, void *dst) {
    wasm_v128_store(dst, src->v128s[0]);
    wasm_v128_store((char *)dst + 16, src->v128s[1]);
}

/** BF16 is the upper 16 bits of F32, so zero-extend to u32 and shift left by 16. */
NUMKONG_HELPER_INLINE nk_b128_vec_t nk_bf16x4_to_f32x4_v128_(nk_b64_vec_t bf16_vec) {
...
NUMKONG_HELPER_INLINE nk_b128_vec_t nk_e5m2x4_to_f32x4_v128_(nk_b32_vec_t e5m2_vec) {
...
/** E2M3 → F32 via Giesen's magic multiply (× 2^126). S EE MMM (bias=1). Shift 5-bit magnitude left
 *  by 20, multiply by 2^126 to rebias. No inf/NaN in E2M3FN format, so no fixup needed. */
NUMKONG_HELPER_INLINE nk_b128_vec_t nk_e2m3x4_to_f32x4_v128_(nk_b32_vec_t e2m3_vec) {
...
/** E3M2 → F32 via Giesen's magic multiply (× 2^124). S EEE MM (bias=3). Shift 5-bit magnitude left
 *  by 21, multiply by 2^124 to rebias. No inf/NaN in E3M2FN format, so no fixup needed. */
NUMKONG_HELPER_INLINE nk_b128_vec_t nk_e3m2x4_to_f32x4_v128_(nk_b32_vec_t e3m2_vec) {
...
/** Convert 4x i8 → f32x4 (WASM). Widen i8 → i16 → i32, convert to f32. */
NUMKONG_HELPER_INLINE nk_b128_vec_t nk_i8x4_to_f32x4_v128_(nk_b32_vec_t in_vec) {
...
/** Convert 4x u8 → f32x4 (WASM). Widen u8 → u16 → u32, convert to f32. */
NUMKONG_HELPER_INLINE nk_b128_vec_t nk_u8x4_to_f32x4_v128_(nk_b32_vec_t in_vec) {
...
/** Convert f32x4 → 4x bf16 via RNE rounding (WASM). */
NUMKONG_HELPER_INLINE nk_b64_vec_t nk_f32x4_to_bf16x4_v128_(nk_b128_vec_t hub_vec) {
...
/** Convert f32x4 → 4x i8 with saturation (WASM). */
NUMKONG_HELPER_INLINE nk_b32_vec_t nk_f32x4_to_i8x4_v128_(nk_b128_vec_t hub_vec) {
...
/** Convert f32x4 → 4x u8 with saturation (WASM). */
NUMKONG_HELPER_INLINE nk_b32_vec_t nk_f32x4_to_u8x4_v128_(nk_b128_vec_t hub_vec) {
