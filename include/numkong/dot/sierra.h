...
NK_HELPER_INLINE void nk_dot_e2m1x64_update_sierra(nk_dot_e2m1x64_state_sierra_t *state, nk_b256_vec_t a,
    ...
    // VPDPBSSD multiplies signed by signed, so no sign fix-up is needed
    state->sum_i32x8 = _mm256_dpbssd_epi32(state->sum_i32x8, a_low_i8x32, b_low_i8x32);
...
NK_API_COMPTIME void nk_dot_e2m1_sierra(nk_e2m1x2_t const *a, nk_e2m1x2_t const *b, nk_size_t n, nk_f32_t *result) {
    ...
    for (; n >= 64; n -= 64, a += 32, b += 32) {
        ...
        nk_dot_e2m1x64_update_sierra(&state, a_vec, b_vec, 0, 64);
    }
    if (n) {
