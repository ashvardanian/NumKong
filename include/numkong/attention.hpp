...
NK_API_COMPTIME void attention_bidirectional_packed(in_type_ const *queries, void const *key_value_packed,
    ...
    if constexpr (std::is_same_v<in_type_, bf16_t> && simd)
        nk_attention_bidirectional_packed_bf16(queries_raw, key_value_packed, output_raw, head_count,
                                               key_value_head_count, depth, query_offsets, query_stride_bytes,
                                               output_stride_bytes, scale, task_start, task_count);
    else if constexpr (std::is_same_v<in_type_, e4m3_t> && simd)
        nk_attention_bidirectional_packed_e4m3(queries_raw, key_value_packed, output_raw, head_count,
                                               key_value_head_count, depth, query_offsets, query_stride_bytes,
                                               output_stride_bytes, scale, task_start, task_count);
    else if constexpr (std::is_same_v<in_type_, i8_t> && simd)
        nk_attention_bidirectional_packed_i8(queries_raw, key_value_packed, output_raw, head_count,
                                             key_value_head_count, depth, query_offsets, query_stride_bytes,
                                             output_stride_bytes, scale, task_start, task_count);
    else if constexpr (std::is_same_v<in_type_, bf16_t>)
        nk_attention_bidirectional_packed_bf16_serial(queries_raw, key_value_packed, output_raw, head_count,
                                                      key_value_head_count, depth, query_offsets, query_stride_bytes,
                                                      output_stride_bytes, scale, task_start, task_count);
    else if constexpr (std::is_same_v<in_type_, e4m3_t>)
        nk_attention_bidirectional_packed_e4m3_serial(queries_raw, key_value_packed, output_raw, head_count,
                                                      key_value_head_count, depth, query_offsets, query_stride_bytes,
                                                      output_stride_bytes, scale, task_start, task_count);
    else if constexpr (std::is_same_v<in_type_, i8_t>)
        nk_attention_bidirectional_packed_i8_serial(queries_raw, key_value_packed, output_raw, head_count,
                                                    key_value_head_count, depth, query_offsets, query_stride_bytes,
                                                    output_stride_bytes, scale, task_start, task_count);
}
...
NK_API_COMPTIME void attention_causal_packed(
    ...
    nk_f32_t *output_raw = reinterpret_cast<nk_f32_t *>(output);
    if constexpr (std::is_same_v<in_type_, bf16_t> && simd)
        nk_attention_causal_packed_bf16(queries_raw, key_value_packed, output_raw, head_count, key_value_head_count,
                                        depth, query_offsets, query_stride_bytes, output_stride_bytes, scale,
                                        diagonal_offset, window, task_start, task_count);
    else if constexpr (std::is_same_v<in_type_, e4m3_t> && simd)
        nk_attention_causal_packed_e4m3(queries_raw, key_value_packed, output_raw, head_count, key_value_head_count,
                                        depth, query_offsets, query_stride_bytes, output_stride_bytes, scale,
                                        diagonal_offset, window, task_start, task_count);
    else if constexpr (std::is_same_v<in_type_, i8_t> && simd)
        nk_attention_causal_packed_i8(queries_raw, key_value_packed, output_raw, head_count, key_value_head_count,
                                      depth, query_offsets, query_stride_bytes, output_stride_bytes, scale,
                                      diagonal_offset, window, task_start, task_count);
    else if constexpr (std::is_same_v<in_type_, bf16_t>)
        nk_attention_causal_packed_bf16_serial(
            ...
            query_stride_bytes, output_stride_bytes, scale, diagonal_offset, window, task_start, task_count);
    else if constexpr (std::is_same_v<in_type_, e4m3_t>)
