 ...
 *
 *  BF16, E4M3, and I8 inputs, F32 outputs, any `head_dim` from 1 to 256: channels are
 *  zero-padded to 32-deep AMX tiles internally (exact math — padded K channels add zero
 *  to every score, padded V channels are never stored back), and `head_dim > 256` routes
 ...
 *
 *  Q, K, V, and O all use the activations-natural layout `[tokens, heads × head_dim]` with
 *  an explicit row stride in bytes: element `(head, token, channel)` lives at
 *  `base + token · stride + (head · head_dim + channel) · sizeof(scalar)`.
 ...
 *  `head_dim = 128`: ~0.63 TFLOPS at `q = kv = 256`, ~0.82 at 1024², ~0.80 at 4096²,
 ...
 *  `[dim_tile][kv_step]` — dim-major so each output tile's depth accumulation streams
 *  contiguous 1 KB tiles (`dim_tile` = 16 channels, `kv_step` = 32 positions).
...
enum {
    ...
    nk_attention_max_head_dim_sapphireamx_k_ = 256,
};
...
typedef void (*nk_attention_gather_sapphireamx_t_)(nk_dots_bf16_a16x32_sapphireamx_t *tile, void const *source,
                                                   nk_size_t source_stride_bytes, nk_size_t valid_rows,
...
typedef void (*nk_attention_v_rows_sapphireamx_t_)(void const *row_a, void const *row_b, int a_live, int b_live,
                                                   __mmask16 columns_mask, __m256i *a_bf16x16, __m256i *b_bf16x16);

NK_INTERNAL void nk_attention_v_rows_bf16_sapphireamx_(void const *row_a, void const *row_b, int a_live, int b_live,
                                                       __mmask16 columns_mask, __m256i *a_bf16x16, __m256i *b_bf16x16) {
    *a_bf16x16 = a_live ? _mm256_maskz_loadu_epi16(columns_mask, row_a) : _mm256_setzero_si256();
    *b_bf16x16 = b_live ? _mm256_maskz_loadu_epi16(columns_mask, row_b) : _mm256_setzero_si256();
}
...
NK_INTERNAL void nk_attention_v_rows_e4m3_sapphireamx_(void const *row_a, void const *row_b, int a_live, int b_live,
                                                       __mmask16 columns_mask, __m256i *a_bf16x16, __m256i *b_bf16x16) {
    __m128i a_e4m3x16 = a_live ? _mm_maskz_loadu_epi8(columns_mask, row_a) : _mm_setzero_si128();
    __m128i b_e4m3x16 = b_live ? _mm_maskz_loadu_epi8(columns_mask, row_b) : _mm_setzero_si128();
...
NK_INTERNAL nk_size_t nk_attention_packed_size_sapphireamx_(nk_size_t num_kv_heads, nk_size_t head_dim,
    ...
    for (nk_size_t s = 0; s < segment_count; s++) {
        nk_size_t const seq_padded = nk_size_round_up_to_multiple_(segment_lengths[s], 32);
        total_tile_bytes += 2 * num_kv_heads * seq_padded * dim_padded * sizeof(nk_bf16_t); // K + V
    }
    return sizeof(nk_attention_packed_header_t) + nk_attention_packed_directory_size_(segment_count) + total_tile_bytes;
}
...
NK_PUBLIC nk_size_t nk_attention_packed_size_bf16_sapphireamx(nk_size_t num_kv_heads, nk_size_t head_dim,
    ...
    if (head_dim > nk_attention_max_head_dim_sapphireamx_k_)
        return nk_attention_packed_size_bf16_serial(num_kv_heads, head_dim, segment_lengths, segment_count);
    return nk_attention_packed_size_sapphireamx_(num_kv_heads, head_dim, segment_lengths, segment_count);
}
...
NK_INTERNAL void nk_attention_pack_sapphireamx_(                                          //
    ...
    nk_size_t const dim_padded = nk_size_round_up_to_multiple_(head_dim, 32);
    nk_size_t const depth_tiles = dim_padded / 32;
    nk_size_t const dim_tiles = dim_padded / 16;

    // Header + directory: deterministic from the arguments, safe to rewrite from any task.
    nk_attention_pack_directory_(kv_packed, num_kv_heads, head_dim, segment_lengths, segment_count, first_task, 32,
                                 dim_padded * sizeof(nk_bf16_t));
    nk_attention_packed_header_t *header = (nk_attention_packed_header_t *)kv_packed;
    nk_u64_t const *tile_offsets_ro = (nk_u64_t const *)((char *)kv_packed + sizeof(*header));
    char *tiles_base = (char *)kv_packed + sizeof(*header) + nk_attention_packed_directory_size_(segment_count);
    ...
    for (nk_size_t task = first_task; task < first_task + task_count; task++) {
        nk_size_t const s = task / num_kv_heads, h = task % num_kv_heads;
        nk_size_t const seq_len = segment_lengths[s];
        if (seq_len == 0) continue;
        nk_size_t const token_first = segment_offsets[s];
        nk_size_t const seq_padded = nk_size_round_up_to_multiple_(seq_len, 32);
        nk_size_t const bytes_per_head = seq_padded * dim_padded * sizeof(nk_bf16_t);
        ...
        nk_bf16_t *k_head_tiles = (nk_bf16_t *)(tiles_base + tile_offsets_ro[s] + h * bytes_per_head);
        for (nk_size_t kv_tile = 0; kv_tile < seq_padded / 16; kv_tile++) {
            nk_size_t const row_start = kv_tile * 16;
            nk_size_t const valid_rows = (row_start + 16 <= seq_len) ? 16
                                         : (row_start < seq_len)     ? seq_len - row_start
                                                                     : 0;
            for (nk_size_t depth_tile = 0; depth_tile < depth_tiles; depth_tile++) {
                nk_size_t const channel_start = depth_tile * 32;
                nk_size_t const valid_columns = (channel_start + 32 <= head_dim) ? 32
                                                : (channel_start < head_dim)     ? head_dim - channel_start
                                                                                 : 0;
                char const *source = (char const *)k + (token_first + row_start) * k_stride +
                                     (h * head_dim + channel_start) * element_bytes;
                nk_dots_bf16_a16x32_sapphireamx_t source_tile;
                nk_dots_bf16_b32x16_sapphireamx_t transposed_tile;
                gather(&source_tile, source, k_stride, valid_rows, valid_columns);
                nk_dots_pack_bf16_transposed_sapphireamx_(&source_tile, &transposed_tile);
                nk_bf16_t *tile_output = k_head_tiles + (kv_tile * depth_tiles + depth_tile) * tile_elements;
        ...
        // VPERMT2W per pair, tiles ordered [dim_tile][kv_step] (dim-major streaming for P×V).
        nk_bf16_t *v_head_tiles = (nk_bf16_t *)(tiles_base + tile_offsets_ro[s] + (num_kv_heads + h) * bytes_per_head);
        for (nk_size_t dim_tile = 0; dim_tile < dim_tiles; dim_tile++) {
            nk_size_t const channel_start = dim_tile * 16;
            nk_size_t const valid_columns = (channel_start + 16 <= head_dim) ? 16
                                            : (channel_start < head_dim)     ? head_dim - channel_start
                                                                             : 0;
            __mmask16 const columns_mask = (__mmask16)((1u << valid_columns) - 1);
            for (nk_size_t kv_step = 0; kv_step < seq_padded / 32; kv_step++) {
                nk_bf16_t *tile_output = v_head_tiles + (dim_tile * (seq_padded / 32) + kv_step) * tile_elements;
                for (nk_size_t pair = 0; pair < 16; pair++) {
                    nk_size_t const row_a = kv_step * 32 + pair * 2, row_b = row_a + 1;
                    char const *row_a_ptr = (char const *)v + (token_first + row_a) * v_stride +
                                            (h * head_dim + channel_start) * element_bytes;
                    char const *row_b_ptr = (char const *)v + (token_first + row_b) * v_stride +
                                            (h * head_dim + channel_start) * element_bytes;
                    __m256i a_bf16x16, b_bf16x16;
                    v_rows(row_a_ptr, row_b_ptr, row_a < seq_len, row_b < seq_len, columns_mask, &a_bf16x16,
                           &b_bf16x16);
                    ...
                    _mm256_storeu_si256((__m256i *)(tile_output + pair * 32), lo_bf16x16);
                    _mm256_storeu_si256((__m256i *)(tile_output + pair * 32 + 16), hi_bf16x16);
                }
            }
        }
    }
...
NK_PUBLIC void nk_attention_pack_bf16_sapphireamx(                    //
    nk_bf16_t const *k, nk_bf16_t const *v,                           //
    nk_size_t num_kv_heads, nk_size_t head_dim,                       //
    nk_u32_t const *segment_offsets, nk_u32_t const *segment_lengths, //
    nk_size_t segment_count,                                          //
    nk_size_t k_stride, nk_size_t v_stride, void *kv_packed,          //
    ...
    nk_attention_pack_sapphireamx_(k, v, sizeof(nk_bf16_t), &nk_attention_gather_bf16_sapphireamx_,
                                   &nk_attention_v_rows_bf16_sapphireamx_, num_kv_heads, head_dim, segment_offsets,
                                   segment_lengths, segment_count, k_stride, v_stride, kv_packed, first_task,
                                   task_count);
}

NK_PUBLIC void nk_attention_pack_e4m3_sapphireamx(                    //
    nk_e4m3_t const *k, nk_e4m3_t const *v,                           //
    nk_size_t num_kv_heads, nk_size_t head_dim,                       //
    nk_u32_t const *segment_offsets, nk_u32_t const *segment_lengths, //
    nk_size_t segment_count,                                          //
    nk_size_t k_stride, nk_size_t v_stride, void *kv_packed,          //
    ...
    nk_attention_pack_sapphireamx_(k, v, sizeof(nk_e4m3_t), &nk_attention_gather_e4m3_sapphireamx_,
                                   &nk_attention_v_rows_e4m3_sapphireamx_, num_kv_heads, head_dim, segment_offsets,
                                   segment_lengths, segment_count, k_stride, v_stride, kv_packed, first_task,
                                   task_count);
}
...
typedef struct {
    ...
    NK_ALIGN64 nk_f32_t o_acc[nk_attention_chunk_sapphireamx_k_][32 * nk_attention_max_head_dim_sapphireamx_k_];
 ...
 *  depth loop covers `depth_tiles` 32-channel steps (4 for the head_dim = 128 hot path).
...
NK_INTERNAL void nk_attention_score_panel_sapphireamx_(        //
    ...
    nk_bf16_t const *k_head_tiles, nk_size_t panel_first_tile, // K tiles, [kv_tile][depth_tile]
    ...
    for (nk_size_t pair = 0; pair < panel_pairs; pair++) {
        nk_bf16_t const *k_tile0 = k_head_tiles + (panel_first_tile + pair * 2) * k_tile_stride;
    ...
    for (nk_size_t group = 0; group < 2; group++)
        for (nk_size_t row = 0; row < 16; row++) {
            nk_f32_t const *scores_row = scores_panel + (group * 16 + row) * panel_width;
            __m512 max_f32x16 = _mm512_set1_ps(NK_F32_MIN);
            for (nk_size_t col = 0; col < panel_pairs * 32; col += 16)
                max_f32x16 = _mm512_max_ps(max_f32x16, _mm512_loadu_ps(scores_row + col));
            panel_max[group][row] = nk_reduce_max_f32x16_skylake_(max_f32x16);
        }
}
...
NK_INTERNAL void nk_attention_exp_panel_sapphireamx_(                  //
    ...
    nk_size_t panel_cols, nk_size_t valid_cols, nk_size_t panel_width, //
    ...
    for (nk_size_t row = 0; row < 32; row++) {
        ...
        nk_size_t col = 0;
        for (; col < full_cols; col += 16) {
            __m512 exp_f32x16 = nk_attention_exp2_ps_skylake_(
                _mm512_fmsub_ps(_mm512_loadu_ps(scores_row + col), scale_f32x16, max_f32x16));
            sum_f32x16 = _mm512_add_ps(sum_f32x16, exp_f32x16);
            _mm256_store_si256((__m256i *)(weights_row + col), (__m256i)_mm512_cvtneps_pbh(exp_f32x16));
        }
        if (col < valid_cols) {
        ...
        for (; col < panel_cols; col += 16) // zero weights for the padded remainder
            _mm256_store_si256((__m256i *)(weights_row + col), _mm256_setzero_si256());
...
NK_INTERNAL void nk_attention_weighted_sum_panel_sapphireamx_( //
    ...
    nk_bf16_t const *v_head_tiles, nk_size_t kv_steps_total,   // V tiles, [dim_tile][kv_step]
    nk_size_t panel_first_step, nk_size_t panel_steps,         //
    nk_size_t dim_slices, nk_size_t o_stride_floats,           // channels / 32, o_acc row stride
    ...
    for (nk_size_t slice = 0; slice < dim_slices; slice++) {
        nk_bf16_t const *v_tile0 = v_head_tiles + ((slice * 2 + 0) * kv_steps_total + panel_first_step) * 512;
        nk_bf16_t const *v_tile1 = v_head_tiles + ((slice * 2 + 1) * kv_steps_total + panel_first_step) * 512;
        _tile_zero(0);
        _tile_zero(1);
        _tile_zero(2);
        _tile_zero(3);
        ...
        for (nk_size_t tile = 0; tile < 4; tile++) {
            ...
            nk_size_t const group = tile / 2;
            nk_size_t const channel_start = slice * 32 + (tile % 2) * 16;
            for (nk_size_t row = 0; row < 16; row++) {
                __m512 const correction_f32x16 = _mm512_set1_ps(corrections[group][row]);
                __m512 acc_f32x16 = _mm512_load_ps(o_acc + (group * 16 + row) * o_stride_floats + channel_start);
                acc_f32x16 = _mm512_fmadd_ps(acc_f32x16, correction_f32x16, _mm512_load_ps(o_panel_tile[row]));
                _mm512_store_ps(o_acc + (group * 16 + row) * o_stride_floats + channel_start, acc_f32x16);
            }
        }
    }
}
...
NK_INTERNAL void nk_attention_task_sapphireamx_(                                             //
    ...
    nk_f32_t *output,                                                                        //
    nk_bf16_t const *k_head_tiles, nk_bf16_t const *v_head_tiles,                            //
    nk_size_t head, nk_size_t head_dim, nk_size_t kv_len, nk_size_t seq_padded,              //
    nk_size_t query_first, nk_size_t query_count,                                            //
    nk_size_t q_stride, nk_size_t o_stride, nk_f32_t scale2,                                 //
    nk_attention_scratch_sapphireamx_t *scratch) {

    nk_size_t const panel_width = nk_attention_panel_sapphireamx_k_;
    nk_size_t const chunk_blocks = nk_attention_chunk_sapphireamx_k_;
    nk_size_t const kv_padded = nk_size_round_up_to_multiple_(kv_len, 32);
    nk_size_t const kv_steps_total = seq_padded / 32;
    nk_size_t const dim_padded = nk_size_round_up_to_multiple_(head_dim, 32);
    nk_size_t const depth_tiles = dim_padded / 32;
    nk_size_t const o_stride_floats = dim_padded;
    nk_size_t const o_stride_out = o_stride / sizeof(nk_f32_t);
    ...
    for (nk_size_t chunk_start = 0; chunk_start < query_count; chunk_start += 32 * chunk_blocks) {
        nk_size_t const blocks = ((query_count - chunk_start + 31) / 32 < chunk_blocks)
                                     ? (query_count - chunk_start + 31) / 32
                                     : chunk_blocks;
        for (nk_size_t block = 0; block < blocks; block++)
            for (nk_size_t group = 0; group < 2; group++) {
                nk_size_t const row_start = chunk_start + block * 32 + group * 16;
                nk_size_t const valid_rows = row_start >= query_count          ? 0
                                             : (query_count - row_start >= 16) ? 16
                                                                               : query_count - row_start;
                for (nk_size_t depth_tile = 0; depth_tile < depth_tiles; depth_tile++) {
                    nk_size_t const channel_start = depth_tile * 32;
                    nk_size_t const valid_columns = (channel_start + 32 <= head_dim) ? 32 : head_dim - channel_start;
                    gather(&scratch->q_tiles[block][group][depth_tile],
                           (char const *)queries + (query_first + row_start) * q_stride +
                               (head * head_dim + channel_start) * element_bytes,
                           q_stride, valid_rows, valid_columns);
                }
                row_max2_f32x16[block][group] = _mm512_set1_ps(NK_F32_MIN);
                row_sum_f32x16[block][group] = _mm512_setzero_ps();
            }
        ...
        for (nk_size_t panel_start = 0; panel_start < kv_padded; panel_start += panel_width) {
            ...
            nk_size_t const valid_cols = (panel_start + panel_cols <= kv_len) ? panel_cols : (kv_len - panel_start);

            for (nk_size_t block = 0; block < blocks; block++) {
                nk_attention_score_panel_sapphireamx_(
                    (nk_dots_bf16_a16x32_sapphireamx_t const(*)[8])scratch->q_tiles[block], k_head_tiles,
                    panel_start / 16, panel_cols / 32, depth_tiles, scratch->scores_panel, panel_width, panel_max);
                for (nk_size_t group = 0; group < 2; group++) {
                    __m512 const old_max_f32x16 = row_max2_f32x16[block][group];
                    __m512 const new_max_f32x16 = _mm512_max_ps(
                        old_max_f32x16, _mm512_mul_ps(_mm512_load_ps(panel_max[group]), scale2_f32x16));
                    __m512 const corrections_f32x16 = nk_attention_exp2_ps_skylake_(
                        _mm512_sub_ps(old_max_f32x16, new_max_f32x16));
                    row_max2_f32x16[block][group] = new_max_f32x16;
                    _mm512_store_ps(corrections[group], corrections_f32x16);
                    _mm512_store_ps(new_max_arr[group], new_max_f32x16);
                }
                nk_attention_exp_panel_sapphireamx_(scratch->scores_panel, scratch->weights_panel, panel_cols,
                                                    ...
                                                    panel_sums);
                for (nk_size_t group = 0; group < 2; group++)
                    row_sum_f32x16[block][group] = _mm512_fmadd_ps(row_sum_f32x16[block][group],
                                                                   _mm512_load_ps(corrections[group]),
                                                                   _mm512_load_ps(panel_sums[group]));
                nk_attention_weighted_sum_panel_sapphireamx_(scratch->weights_panel, panel_width, v_head_tiles,
                                                             kv_steps_total, panel_start / 32, panel_cols / 32,
                                                             depth_tiles, o_stride_floats,
                                                             (nk_f32_t const(*)[16])corrections, scratch->o_acc[block]);
            }
        }
        ...
        for (nk_size_t block = 0; block < blocks; block++)
            for (nk_size_t group = 0; group < 2; group++) {
                nk_size_t const row_start = chunk_start + block * 32 + group * 16;
                if (row_start >= query_count) break;
                nk_size_t const valid_rows = (query_count - row_start >= 16) ? 16 : query_count - row_start;
                NK_ALIGN64 nk_f32_t row_sums[16];
                _mm512_store_ps(row_sums, row_sum_f32x16[block][group]);
                for (nk_size_t row = 0; row < valid_rows; row++) {
                    __m512 const inv_sum_f32x16 = _mm512_set1_ps(1.0f / row_sums[row]);
                    nk_f32_t const *acc_row = &scratch->o_acc[block][(group * 16 + row) * o_stride_floats];
                    nk_f32_t *output_row = output + (query_first + row_start + row) * o_stride_out + head * head_dim;
                    nk_size_t col = 0;
                    for (; col < dim_full; col += 16)
                        _mm512_storeu_ps(output_row + col,
                                         _mm512_mul_ps(_mm512_load_ps(acc_row + col), inv_sum_f32x16));
                    if (col < head_dim)
                        _mm512_mask_storeu_ps(output_row + col, dim_tail_mask,
                                              _mm512_mul_ps(_mm512_load_ps(acc_row + col), inv_sum_f32x16));
                }
                for (nk_size_t row = 0; row < 16; row++)
                    for (nk_size_t col = 0; col < o_stride_floats; col += 16)
                        _mm512_store_ps(&scratch->o_acc[block][(group * 16 + row) * o_stride_floats + col],
                                        zero_f32x16);
            }
    }
}
...
NK_INTERNAL void nk_attention_packed_sapphireamx_(                                           //
    ...
    nk_u64_t const *tile_offsets = (nk_u64_t const *)((char const *)kv_packed + sizeof(*header));
    nk_u32_t const *segment_lengths = (nk_u32_t const *)(tile_offsets + segment_count + 1);
    char const *tiles_base = (char const *)kv_packed + sizeof(*header) +
                             nk_attention_packed_directory_size_(segment_count);
    nk_size_t const gqa_ratio = num_heads / num_kv_heads;
    nk_size_t const dim_padded = nk_size_round_up_to_multiple_(head_dim, 32);
    nk_f32_t const scale2 = scale * NK_LOG2E_; // fold log2e: softmax(x) = softmax₂(x·log₂e)
    ...
    __m512 const zero_f32x16 = _mm512_setzero_ps();
    for (nk_size_t block = 0; block < nk_attention_chunk_sapphireamx_k_; block++)
        for (nk_size_t i = 0; i < 32 * nk_attention_max_head_dim_sapphireamx_k_; i += 16)
            _mm512_store_ps(&scratch.o_acc[block][i], zero_f32x16);
    ...
    for (nk_size_t task = first_task; task < first_task + task_count; task++) {
        ...
        nk_size_t const seq_padded = nk_size_round_up_to_multiple_(kv_len, 32);
        nk_size_t const bytes_per_head = seq_padded * dim_padded * sizeof(nk_bf16_t);
        nk_size_t const kv_head = head / gqa_ratio;
        nk_bf16_t const *k_head_tiles = (nk_bf16_t const *)(tiles_base + tile_offsets[segment] +
                                                            kv_head * bytes_per_head);
        nk_bf16_t const *v_head_tiles = (nk_bf16_t const *)(tiles_base + tile_offsets[segment] +
                                                            (num_kv_heads + kv_head) * bytes_per_head);
        nk_attention_task_sapphireamx_(queries, element_bytes, gather, output, k_head_tiles, v_head_tiles, head,
                                       head_dim, kv_len, seq_padded, query_offsets[segment], query_count, q_stride,
                                       o_stride, scale2, &scratch);
    }
    _tile_release();
}
...
NK_PUBLIC void nk_attention_packed_bf16_sapphireamx(                   //
    ...
    nk_attention_packed_sapphireamx_(queries, sizeof(nk_bf16_t), &nk_attention_gather_bf16_sapphireamx_, kv_packed,
                                     output, num_heads, num_kv_heads, head_dim, query_offsets, q_stride, o_stride,
...
NK_PUBLIC void nk_attention_packed_e4m3_sapphireamx(                   //
    ...
    nk_attention_packed_sapphireamx_(queries, sizeof(nk_e4m3_t), &nk_attention_gather_e4m3_sapphireamx_, kv_packed,
                                     output, num_heads, num_kv_heads, head_dim, query_offsets, q_stride, o_stride,
                                     scale, first_task, task_count);
}
...
NK_PUBLIC nk_size_t nk_attention_packed_size_i8_sapphireamx(nk_size_t num_kv_heads, nk_size_t head_dim,
    ...
    nk_size_t const dim_padded = nk_size_round_up_to_multiple_(head_dim, 64);
    nk_size_t total_tile_bytes = 0;
    for (nk_size_t s = 0; s < segment_count; s++) {
        nk_size_t const seq_padded = nk_size_round_up_to_multiple_(segment_lengths[s], 64);
        total_tile_bytes += 2 * num_kv_heads * seq_padded * dim_padded; // K + V, one byte per element
    }
    return sizeof(nk_attention_packed_header_t) + nk_attention_packed_directory_size_(segment_count) + total_tile_bytes;
}

NK_PUBLIC void nk_attention_pack_i8_sapphireamx(                      //
    nk_i8_t const *k, nk_i8_t const *v,                               //
    nk_size_t num_kv_heads, nk_size_t head_dim,                       //
    nk_u32_t const *segment_offsets, nk_u32_t const *segment_lengths, //
    nk_size_t segment_count,                                          //
    nk_size_t k_stride, nk_size_t v_stride, void *kv_packed,          //
    nk_size_t first_task, nk_size_t task_count) {
    if (head_dim > nk_attention_max_head_dim_sapphireamx_k_) {
        nk_attention_pack_i8_serial(k, v, num_kv_heads, head_dim, segment_offsets, segment_lengths, segment_count,
                                    k_stride, v_stride, kv_packed, first_task, task_count);
        return;
    }

    nk_size_t const dim_padded = nk_size_round_up_to_multiple_(head_dim, 64);
    nk_size_t const depth_tiles = dim_padded / 64; // I8 tiles are 64 channels deep, not 32
    nk_size_t const dim_tiles = dim_padded / 16;
    nk_size_t const tile_bytes = 1024;

    nk_attention_pack_directory_(kv_packed, num_kv_heads, head_dim, segment_lengths, segment_count, first_task, 64,
                                 dim_padded);
    nk_attention_packed_header_t *header = (nk_attention_packed_header_t *)kv_packed;
    nk_u64_t const *tile_offsets_ro = (nk_u64_t const *)((char *)kv_packed + sizeof(*header));
    char *tiles_base = (char *)kv_packed + sizeof(*header) + nk_attention_packed_directory_size_(segment_count);
    ...
    for (nk_size_t task = first_task; task < first_task + task_count; task++) {
        ...
        if (seq_len == 0) continue;
        nk_size_t const token_first = segment_offsets[s];
        nk_size_t const seq_padded = nk_size_round_up_to_multiple_(seq_len, 64);
        nk_size_t const bytes_per_head = seq_padded * dim_padded;
        ...
        nk_i8_t *k_head_tiles = (nk_i8_t *)(tiles_base + tile_offsets_ro[s] + h * bytes_per_head);
        for (nk_size_t kv_tile = 0; kv_tile < seq_padded / 16; kv_tile++) {
            nk_size_t const row_start = kv_tile * 16;
            nk_size_t const valid_rows = (row_start + 16 <= seq_len) ? 16
                                         : (row_start < seq_len)     ? seq_len - row_start
            ...
            for (nk_size_t depth_tile = 0; depth_tile < depth_tiles; depth_tile++) {
                ...
                nk_size_t const valid_columns = (channel_start + 64 <= head_dim) ? 64
                                                : (channel_start < head_dim)     ? head_dim - channel_start
                                                                                 : 0;
                nk_i8_t const *source = (nk_i8_t const *)((char const *)k + (token_first + row_start) * k_stride) +
                                        h * head_dim + channel_start;
                nk_dots_i8_a16x64_sapphireamx_t source_tile;
                nk_dots_i8_b64x16_sapphireamx_t transposed_tile;
                nk_dots_i8_load_a_sapphireamx_(&source_tile, source, k_stride, valid_rows, valid_columns);
                nk_dots_pack_i8_transposed_sapphireamx_(&source_tile, &transposed_tile);
                nk_i8_t *tile_output = k_head_tiles + (kv_tile * depth_tiles + depth_tile) * tile_bytes;
        ...
        // VPERMB per quad, tiles ordered [dim_tile][kv_step] (dim-major streaming for P×V).
        nk_i8_t *v_head_tiles = (nk_i8_t *)(tiles_base + tile_offsets_ro[s] + (num_kv_heads + h) * bytes_per_head);
        for (nk_size_t dim_tile = 0; dim_tile < dim_tiles; dim_tile++) {
            nk_size_t const channel_start = dim_tile * 16;
            nk_size_t const valid_columns = (channel_start + 16 <= head_dim) ? 16
                                            : (channel_start < head_dim)     ? head_dim - channel_start
                                                                             : 0;
            __mmask16 const columns_mask = (__mmask16)((1u << valid_columns) - 1);
            for (nk_size_t kv_step = 0; kv_step < seq_padded / 64; kv_step++) {
                nk_i8_t *tile_output = v_head_tiles + (dim_tile * (seq_padded / 64) + kv_step) * tile_bytes;
                for (nk_size_t quad = 0; quad < 16; quad++) {
                    nk_size_t const row_first = kv_step * 64 + quad * 4;
                    __m512i quad_i8x64 = _mm512_setzero_si512();
                    for (nk_size_t r = 0; r < 4; r++) {
                        if (row_first + r >= seq_len) continue;
                        char const *row_ptr = (char const *)v + (token_first + row_first + r) * v_stride +
                                              h * head_dim + channel_start;
...
NK_INTERNAL void nk_attention_score_panel_i8_sapphireamx_(   //
    ...
    nk_i8_t const *k_head_tiles, nk_size_t panel_first_tile, // K tiles, [kv_tile][depth_tile]
    ...
    nk_size_t valid_cols, nk_f32_t (*panel_max)[16]) {       // live columns, [2][16] raw maxima out

    int const panel_stride_bytes = (int)(panel_width * sizeof(nk_i32_t));
    nk_size_t const k_tile_stride = depth_tiles * 1024;
    for (nk_size_t pair = 0; pair < panel_pairs; pair++) {
        nk_i8_t const *k_tile0 = k_head_tiles + (panel_first_tile + pair * 2) * k_tile_stride;
    ...
    for (nk_size_t group = 0; group < 2; group++)
        for (nk_size_t row = 0; row < 16; row++) {
            nk_i32_t const *scores_row = scores_panel + (group * 16 + row) * panel_width;
            __m512 max_f32x16 = _mm512_set1_ps(NK_F32_MIN);
            nk_size_t col = 0;
            for (; col < full_cols; col += 16)
                max_f32x16 = _mm512_max_ps(max_f32x16, _mm512_cvtepi32_ps(_mm512_load_si512(scores_row + col)));
            if (col < valid_cols)
                max_f32x16 = _mm512_max_ps(max_f32x16,
                                           _mm512_mask_mov_ps(_mm512_set1_ps(NK_F32_MIN), tail_mask,
                                                              _mm512_cvtepi32_ps(_mm512_load_si512(scores_row + col))));
            panel_max[group][row] = nk_reduce_max_f32x16_skylake_(max_f32x16);
        }
}

/**
 *  @brief Stage 2: streaming base-2 softmax with U8 weight quantization over one panel.
 *
 *  Row-major single pass: `w̃ = (u8)(2^(score·scale₂ − m₂_row) · 255 + 0.5)` — separate
 *  multiply and add to match the serial reference's rounding exactly — with the running
 *  sum of the @b quantized weights in a register.  The max-scoring position lands on 255,
 *  so the sum can never be zero.  Columns past `valid_cols` get zero weights (U8 zero is
 *  an exact zero weight) and no sum contribution.
 */
    ...
    for (nk_size_t row = 0; row < 32; row++) {
        ...
        for (; col < full_cols; col += 16) {
            __m512 const exp_f32x16 = nk_attention_exp2_ps_skylake_(
                _mm512_fmsub_ps(_mm512_cvtepi32_ps(_mm512_load_si512(scores_row + col)), scale_f32x16, max_f32x16));
            ...
            _mm_store_si128((__m128i *)(weights_row + col), _mm512_cvtusepi32_epi8(weight_u32x16));
        }
        if (col < valid_cols) {
            __m512 const exp_f32x16 = _mm512_maskz_mov_ps(
                tail_mask, nk_attention_exp2_ps_skylake_(_mm512_fmsub_ps(
                               _mm512_cvtepi32_ps(_mm512_load_si512(scores_row + col)), scale_f32x16, max_f32x16)));
            __m512i const weight_u32x16 = _mm512_cvttps_epu32(
                _mm512_add_ps(_mm512_mul_ps(exp_f32x16, amplitude_f32x16), half_f32x16));
            sum_f32x16 = _mm512_add_ps(sum_f32x16, _mm512_cvtepu32_ps(weight_u32x16));
            _mm_store_si128((__m128i *)(weights_row + col), _mm512_cvtusepi32_epi8(weight_u32x16));
        ...
        for (; col < panel_cols; col += 16) // zero weights for the padded remainder
            _mm_store_si128((__m128i *)(weights_row + col), _mm_setzero_si128());
        panel_sums[row / 16][row % 16] = nk_reduce_add_f32x16_skylake_(sum_f32x16);
    }
}
...
NK_INTERNAL void nk_attention_weighted_sum_panel_i8_sapphireamx_( //
    ...
    nk_i8_t const *v_head_tiles, nk_size_t kv_steps_total,        // V tiles, [dim_tile][kv_step]
    ...
    for (nk_size_t slice = 0; slice < dim_slices; slice++) {
        nk_i8_t const *v_tile0 = v_head_tiles + ((slice * 2 + 0) * kv_steps_total + panel_first_step) * 1024;
        nk_i8_t const *v_tile1 = v_head_tiles + ((slice * 2 + 1) * kv_steps_total + panel_first_step) * 1024;
        _tile_zero(0);
        _tile_zero(1);
        _tile_zero(2);
        _tile_zero(3);
        ...
        for (nk_size_t tile = 0; tile < 4; tile++) {
            ...
            for (nk_size_t row = 0; row < 16; row++) {
                __m512 const correction_f32x16 = _mm512_set1_ps(corrections[group][row]);
                __m512 acc_f32x16 = _mm512_load_ps(o_acc + (group * 16 + row) * o_stride_floats + channel_start);
                acc_f32x16 = _mm512_fmadd_ps(acc_f32x16, correction_f32x16,
...
NK_INTERNAL void nk_attention_task_i8_sapphireamx_(                             //
    ...
    nk_i8_t const *k_head_tiles, nk_i8_t const *v_head_tiles,                   //
    nk_size_t head, nk_size_t head_dim, nk_size_t kv_len, nk_size_t seq_padded, //
    nk_size_t query_first, nk_size_t query_count,                               //
    nk_size_t q_stride, nk_size_t o_stride, nk_f32_t scale2,                    //
    ...
    nk_size_t const panel_width = nk_attention_panel_sapphireamx_k_;
    nk_size_t const chunk_blocks = nk_attention_chunk_sapphireamx_k_;
    nk_size_t const kv_padded = nk_size_round_up_to_multiple_(kv_len, 64);
    nk_size_t const kv_steps_total = seq_padded / 64;
    nk_size_t const dim_padded = nk_size_round_up_to_multiple_(head_dim, 64);
    nk_size_t const depth_tiles = dim_padded / 64;
    nk_size_t const dim_slices = dim_padded / 32;
    nk_size_t const o_stride_floats = dim_padded;
    nk_size_t const o_stride_out = o_stride / sizeof(nk_f32_t);
    ...
    for (nk_size_t chunk_start = 0; chunk_start < query_count; chunk_start += 32 * chunk_blocks) {
        ...
        for (nk_size_t block = 0; block < blocks; block++)
            for (nk_size_t group = 0; group < 2; group++) {
                ...
                for (nk_size_t depth_tile = 0; depth_tile < depth_tiles; depth_tile++) {
                    nk_size_t const channel_start = depth_tile * 64;
                    nk_size_t const valid_columns = (channel_start + 64 <= head_dim) ? 64 : head_dim - channel_start;
                    nk_dots_i8_load_a_sapphireamx_(
                        &scratch->q_tiles[block][group][depth_tile],
                        (nk_i8_t const *)((char const *)queries + (query_first + row_start) * q_stride) +
                            head * head_dim + channel_start,
                        q_stride, valid_rows, valid_columns);
                }
                row_max2_f32x16[block][group] = _mm512_set1_ps(NK_F32_MIN);
                row_sum_f32x16[block][group] = _mm512_setzero_ps();
        ...
        for (nk_size_t panel_start = 0; panel_start < kv_padded; panel_start += panel_width) {
            ...
            for (nk_size_t block = 0; block < blocks; block++) {
                nk_attention_score_panel_i8_sapphireamx_(
                    (nk_dots_i8_a16x64_sapphireamx_t const(*)[4])scratch->q_tiles[block], k_head_tiles,
                    panel_start / 16, panel_cols / 32, depth_tiles, scratch->scores_panel, panel_width, valid_cols,
                    panel_max);
                for (nk_size_t group = 0; group < 2; group++) {
                    __m512 const old_max_f32x16 = row_max2_f32x16[block][group];
                    __m512 const new_max_f32x16 = _mm512_max_ps(
                ...
                nk_attention_weighted_sum_panel_i8_sapphireamx_(
                    scratch->weights_panel, panel_width, v_head_tiles, kv_steps_total, panel_start / 64,
                    panel_cols / 64, dim_slices, o_stride_floats, (nk_f32_t const(*)[16])corrections,
                    scratch->o_acc[block]);
            }
        }
...
NK_PUBLIC void nk_attention_packed_i8_sapphireamx(                   //
    nk_i8_t const *queries, void const *kv_packed, nk_f32_t *output, //
    nk_size_t num_heads, nk_size_t num_kv_heads, nk_size_t head_dim, //
    nk_u32_t const *query_offsets,                                   //
    nk_size_t q_stride, nk_size_t o_stride, nk_f32_t scale,          //
    nk_size_t first_task, nk_size_t task_count) {
    if (head_dim > nk_attention_max_head_dim_sapphireamx_k_) {
        nk_attention_packed_i8_serial(queries, kv_packed, output, num_heads, num_kv_heads, head_dim, query_offsets,
                                      q_stride, o_stride, scale, first_task, task_count);
        return;
    }
    ...
    for (nk_size_t task = first_task; task < first_task + task_count; task++) {
        ...
        nk_size_t const query_count = query_offsets[segment + 1] - query_offsets[segment];
        if (kv_len == 0 || query_count == 0) continue;
        nk_size_t const seq_padded = nk_size_round_up_to_multiple_(kv_len, 64);
        nk_size_t const bytes_per_head = seq_padded * dim_padded;
        nk_size_t const kv_head = head / gqa_ratio;
        nk_i8_t const *k_head_tiles = (nk_i8_t const *)(tiles_base + tile_offsets[segment] + kv_head * bytes_per_head);
        nk_i8_t const *v_head_tiles = (nk_i8_t const *)(tiles_base + tile_offsets[segment] +
                                                        (num_kv_heads + kv_head) * bytes_per_head);
        nk_attention_task_i8_sapphireamx_(queries, output, k_head_tiles, v_head_tiles, head, head_dim, kv_len,
                                          seq_padded, query_offsets[segment], query_count, q_stride, o_stride, scale2,
                                          &scratch);
    }
    _tile_release();
}
