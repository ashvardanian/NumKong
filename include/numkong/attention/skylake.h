 ...
 *  ports.  Packed payload per segment: K planes then V planes, `[kv_head][token][channel]`
 *  in 16-bit scalars with channels zero-padded to a multiple of 16.  `head_dim > 256`
...
enum {
    ...
    nk_attention_max_head_dim_skylake_k_ = 256,
};

/** @brief Fast vectorized 2^x: exact range reduction + the family's shared degree-4 polynomial. */
NK_INTERNAL __m512 nk_attention_exp2_ps_skylake_(__m512 x_f32x16) {
 ...
 */
NK_INTERNAL nk_f32_t nk_attention_softmax_panel_skylake_(nk_f32_t *scores, nk_size_t panel_len, nk_f32_t scale2,
                                                         nk_f32_t *running_max2, nk_f32_t *running_sum) {
    __m512 max_f32x16 = _mm512_set1_ps(NK_F32_MIN);
    nk_size_t col = 0;
    for (; col + 16 <= panel_len; col += 16) max_f32x16 = _mm512_max_ps(max_f32x16, _mm512_loadu_ps(scores + col));
    ...
    *running_max2 = new_max2;
    ...
    nk_size_t const panel_full = panel_len & ~(nk_size_t)15;
    __mmask16 const panel_tail_mask = (__mmask16)((1u << (panel_len - panel_full)) - 1);
    for (col = 0; col < panel_full; col += 16) {
        __m512 weights_f32x16 = nk_attention_exp2_ps_skylake_(
            _mm512_fmsub_ps(_mm512_loadu_ps(scores + col), scale2_f32x16, max2_f32x16));
        sum_f32x16 = _mm512_add_ps(sum_f32x16, weights_f32x16);
        _mm512_storeu_ps(scores + col, weights_f32x16);
    }
    if (col < panel_len) {
        __m512 weights_f32x16 = _mm512_maskz_mov_ps(
            panel_tail_mask,
            nk_attention_exp2_ps_skylake_(_mm512_fmsub_ps(_mm512_loadu_ps(scores + col), scale2_f32x16, max2_f32x16)));
        sum_f32x16 = _mm512_add_ps(sum_f32x16, weights_f32x16);
        _mm512_storeu_ps(scores + col, weights_f32x16);
    }
...
typedef __m512 (*nk_attention_plane_widen_skylake_t_)(void const *plane_chunk);

NK_INTERNAL __m512 nk_attention_plane_widen_bf16_skylake_(void const *plane_chunk) {
    return nk_bf16x16_to_f32x16_skylake_(_mm256_loadu_si256((__m256i const *)plane_chunk));
}

NK_INTERNAL __m512 nk_attention_plane_widen_f16_skylake_(void const *plane_chunk) {
    return _mm512_cvtph_ps(_mm256_loadu_si256((__m256i const *)plane_chunk));
}
...
NK_INTERNAL void nk_attention_narrow_bf16_skylake_(void const *source, void *destination, nk_size_t count,
                                                   nk_size_t padded) {
    nk_size_t channel = 0; // BF16 stays BF16 at rest, like the dots family
    for (; channel + 16 <= count; channel += 16)
        _mm256_storeu_si256((__m256i *)((nk_bf16_t *)destination + channel),
                            _mm256_loadu_si256((__m256i const *)((nk_bf16_t const *)source + channel)));
    if (channel < count) {
        __mmask16 const mask = (__mmask16)((1u << (count - channel)) - 1);
        _mm256_storeu_si256((__m256i *)((nk_bf16_t *)destination + channel),
                            _mm256_maskz_loadu_epi16(mask, (nk_bf16_t const *)source + channel));
        channel += 16;
    }
    for (; channel < padded; channel += 16)
        _mm256_storeu_si256((__m256i *)((nk_bf16_t *)destination + channel), _mm256_setzero_si256());
}

NK_INTERNAL void nk_attention_narrow_e4m3_skylake_(void const *source, void *destination, nk_size_t count,
                                                   nk_size_t padded) {
    nk_size_t channel = 0; // E4M3 converts once to F16, so the hot loops widen with one VCVTPH2PS
    nk_b256_vec_t converted;
    for (; channel + 16 <= count; channel += 16) {
        nk_load_e4m3x16_to_f16x16_skylake_((nk_e4m3_t const *)source + channel, &converted);
        _mm256_storeu_si256((__m256i *)((nk_f16_t *)destination + channel), converted.ymm);
    }
    if (channel < count) {
        nk_partial_load_e4m3x16_to_f16x16_skylake_((nk_e4m3_t const *)source + channel, &converted, count - channel);
        _mm256_storeu_si256((__m256i *)((nk_f16_t *)destination + channel), converted.ymm);
        channel += 16;
    }
    for (; channel < padded; channel += 16)
        _mm256_storeu_si256((__m256i *)((nk_f16_t *)destination + channel), _mm256_setzero_si256());
}
...
NK_INTERNAL void nk_attention_widen_bf16_skylake_(void const *source, nk_f32_t *destination, nk_size_t count,
    ...
    for (; channel + 16 <= count; channel += 16) {
        nk_load_bf16x16_to_f32x16_skylake_((nk_bf16_t const *)source + channel, &widened);
        _mm512_storeu_ps(destination + channel, widened.zmm_ps);
    }
    if (channel < count) {
        nk_partial_load_bf16x16_to_f32x16_skylake_((nk_bf16_t const *)source + channel, &widened, count - channel);
        _mm512_storeu_ps(destination + channel, widened.zmm_ps);
        channel += 16;
    }
    for (; channel < padded; channel += 16) _mm512_storeu_ps(destination + channel, _mm512_setzero_ps());
}
...
NK_INTERNAL void nk_attention_widen_e4m3_skylake_(void const *source, nk_f32_t *destination, nk_size_t count,
    ...
    nk_b512_vec_t widened;
    for (; channel + 16 <= count; channel += 16) {
        nk_load_e4m3x16_to_f32x16_skylake_((nk_e4m3_t const *)source + channel, &widened);
        _mm512_storeu_ps(destination + channel, widened.zmm_ps);
    }
    if (channel < count) {
        nk_partial_load_e4m3x16_to_f32x16_skylake_((nk_e4m3_t const *)source + channel, &widened, count - channel);
        _mm512_storeu_ps(destination + channel, widened.zmm_ps);
        channel += 16;
    }
    for (; channel < padded; channel += 16) _mm512_storeu_ps(destination + channel, _mm512_setzero_ps());
...
NK_INTERNAL nk_size_t nk_attention_packed_size_skylake_(nk_size_t num_kv_heads, nk_size_t head_dim,
                                                        nk_u32_t const *segment_lengths, nk_size_t segment_count) {
    nk_size_t const dim_padded = nk_size_round_up_to_multiple_(head_dim, 16);
    nk_size_t payload_bytes = 0; // planes hold 16-bit scalars for both dtypes (BF16 or F16)
    for (nk_size_t s = 0; s < segment_count; s++)
        payload_bytes += 2 * num_kv_heads * (nk_size_t)segment_lengths[s] * dim_padded * sizeof(nk_bf16_t);
...
NK_PUBLIC nk_size_t nk_attention_packed_size_bf16_skylake(nk_size_t num_kv_heads, nk_size_t head_dim,
                                                          nk_u32_t const *segment_lengths, nk_size_t segment_count) {
    if (head_dim > nk_attention_max_head_dim_skylake_k_)
        return nk_attention_packed_size_bf16_serial(num_kv_heads, head_dim, segment_lengths, segment_count);
    return nk_attention_packed_size_skylake_(num_kv_heads, head_dim, segment_lengths, segment_count);
}
...
NK_INTERNAL void nk_attention_pack_skylake_(                                          //
    ...
    for (nk_size_t task = first_task; task < first_task + task_count; task++) {
        ...
        nk_size_t const plane_bytes = seq_len * dim_padded * sizeof(nk_bf16_t);
        ...
        for (nk_size_t token = 0; token < seq_len; token++) {
            narrow((char const *)k + (token_first + token) * k_stride + kv_head * head_dim * element_bytes,
                   k_plane + token * dim_padded * sizeof(nk_bf16_t), head_dim, dim_padded);
            narrow((char const *)v + (token_first + token) * v_stride + kv_head * head_dim * element_bytes,
                   v_plane + token * dim_padded * sizeof(nk_bf16_t), head_dim, dim_padded);
        }
    }
}
...
NK_PUBLIC void nk_attention_pack_bf16_skylake(                                          //
    ...
    nk_attention_pack_skylake_(k, v, sizeof(nk_bf16_t), &nk_attention_narrow_bf16_skylake_, num_kv_heads, head_dim,
...
NK_PUBLIC void nk_attention_pack_e4m3_skylake(                                          //
    ...
    nk_attention_pack_skylake_(k, v, sizeof(nk_e4m3_t), &nk_attention_narrow_e4m3_skylake_, num_kv_heads, head_dim,
                               segment_offsets, segment_lengths, segment_count, k_stride, v_stride, kv_packed,
                               first_task, task_count);
}
...
NK_INTERNAL void nk_attention_packed_skylake_(                                             //
    ...
    nk_attention_plane_widen_skylake_t_ plane_widen,                                       //
    ...
    nk_size_t const dim_padded = nk_size_round_up_to_multiple_(head_dim, 16);
    nk_size_t const plane_row_bytes = dim_padded * sizeof(nk_bf16_t);
    ...
    NK_ALIGN64 nk_f32_t query_row[nk_attention_max_head_dim_skylake_k_];
    NK_ALIGN64 nk_f32_t output_row[nk_attention_max_head_dim_skylake_k_];
        ...
        for (nk_size_t row = 0; row < query_count; row++) {
            ...
            for (nk_size_t panel_start = 0; panel_start < kv_len; panel_start += panel_width) {
                ...
                for (; position + 4 <= panel_len; position += 4) {
                    ...
                    for (nk_size_t channel = 0; channel < dim_padded; channel += 16) {
                        __m512 const q_f32x16 = _mm512_load_ps(query_row + channel);
                        nk_size_t const chunk_bytes = channel * sizeof(nk_bf16_t);
                        acc0_f32x16 = _mm512_fmadd_ps(q_f32x16, plane_widen(k_row + chunk_bytes), acc0_f32x16);
                        acc1_f32x16 = _mm512_fmadd_ps(q_f32x16, plane_widen(k_row + plane_row_bytes + chunk_bytes),
                                                      acc1_f32x16);
                        acc2_f32x16 = _mm512_fmadd_ps(q_f32x16, plane_widen(k_row + 2 * plane_row_bytes + chunk_bytes),
                                                      acc2_f32x16);
                        acc3_f32x16 = _mm512_fmadd_ps(q_f32x16, plane_widen(k_row + 3 * plane_row_bytes + chunk_bytes),
                                                      acc3_f32x16);
                    }
                ...
                for (; position < panel_len; position++) {
                    ...
                    for (nk_size_t channel = 0; channel < dim_padded; channel += 16)
                        acc_f32x16 = _mm512_fmadd_ps(_mm512_load_ps(query_row + channel),
                                                     plane_widen(k_row + channel * sizeof(nk_bf16_t)), acc_f32x16);
                ...
                for (position = 0; position < panel_len; position++) {
                    ...
                    for (nk_size_t channel = 0; channel < dim_padded; channel += 16)
                        _mm512_store_ps(output_row + channel,
                                        _mm512_fmadd_ps(weight_f32x16, plane_widen(v_row + channel * sizeof(nk_bf16_t)),
                                                        _mm512_load_ps(output_row + channel)));
                }
            }
...
NK_PUBLIC void nk_attention_packed_bf16_skylake(                       //
    ...
    nk_attention_packed_skylake_(queries, sizeof(nk_bf16_t), &nk_attention_widen_bf16_skylake_,
                                 &nk_attention_plane_widen_bf16_skylake_, kv_packed, output, num_heads, num_kv_heads,
...
NK_PUBLIC void nk_attention_packed_e4m3_skylake(                       //
    ...
    nk_attention_packed_skylake_(queries, sizeof(nk_e4m3_t), &nk_attention_widen_e4m3_skylake_,
                                 &nk_attention_plane_widen_f16_skylake_, kv_packed, output, num_heads, num_kv_heads,
                                 head_dim, query_offsets, q_stride, o_stride, scale, first_task, task_count);
}
