...
Every backend shares one public triple — `packed_size` to size the cache, `pack` to rearrange K/V into a backend-opaque layout, and `packed` to compute attention against it — with `(first_task, task_count)` windows over the segment × head grid for embarrassingly parallel execution.
