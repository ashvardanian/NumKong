...
let package = Package(
    ...
    targets: [
        ...
        // `path` must contain every entry in `sources` — SPM silently resolves zero sources
        // for paths that escape it, so the root is the target directory and `sources` selects.
        .target(
            name: "CNumKong",
            path: ".",
            sources: [
